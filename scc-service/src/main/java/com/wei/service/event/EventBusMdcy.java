 ...
 * 是否MDCY
 */
public class EventBusMdcy implements Observer {
    @Override
...
//                .filter(m -> "MDCY".equals(m.getAdrgCode()))
//                .collect(Collectors.toList());
//        //以P开始的所有的ADRG列表
//        List<MdcAdrgDiag> mdcAdrgDiags = args.getMdcAdrgDiagList().stream()
//                .filter(m -> m.getAdrgCode().startsWith("Y"))
//                .collect(Collectors.toList());
//
//        //获取第二布 全部手术/操作的ADRG
...
//
//        if (Arrays.asList(mainResult.split(AppConstant.COMMA)).stream().anyMatch(m -> m.startsWith("Y"))
//                || Arrays.asList(allResult.split(AppConstant.COMMA)).stream().anyMatch(m -> m.startsWith("Y"))
//                || Arrays.asList(operationCodes.split(AppConstant.COMMA)).stream().anyMatch(m -> m.startsWith("Y"))) {
//
